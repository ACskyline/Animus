...
void parseFace(std::stringstream &ss, GLuint vIndices[3], GLuint tIndices[3], GLuint nIndices[3])
{
	char discard;
	char peek;
	int count;
...
int loadObj(char* fileName, std::vector<glm::vec4> &vertices, std::vector<glm::vec2> &texcoords,std::vector<glm::vec3> &normals,  std::vector<GLuint> &vertexIndices, std::vector<GLuint> &texcoordIndices, std::vector<GLuint> &normalIndices)
{
	std::fstream file;
...
int main()
{
	...
	std::vector<GLuint> TI;
	std::vector<GLuint> NI;
	char *file = "animus3.obj";

	
	loadObj(file, V, T, N, VI, TI, NI);
